// ==UserScript==
// @name        DARBS.LT Citation Generator for Wikipedia
// @namespace   https://github.com/zygimantus/vikipedijos-irankiai
// @description Generates Wikipedia {{cite web}} references automatically from darbs.lt articles
// @match       https://darbs.lt/*
// @match       https://www.darbs.lt/*
// @version     1.0.0
// @author      Zygimantus
// @icon        https://www.darbs.lt/favicon.ico
// @run-at      document-end
// @noframes    
// @downloadURL https://zygimantus.github.io/vikipedijos-irankiai/scripts/cite-web-darbs.user.js
// @updateURL   https://zygimantus.github.io/vikipedijos-irankiai/scripts/cite-web-darbs.user.js
...
generate({
  title: '.single_title h1',
  date: '.singe__post-meta ul.d-flex li:nth-child(2)',
  dateFormat: raw => {
    if (!raw) return raw;
    const date = raw.replace(',', '').trim();
    const parts = date.split(/\s+/);
    if (parts.length === 3) {
      const [year, day, monthLt] = parts;
  ...
  author: '.authore',
  publisher: '[[Darbas (Pasvalys)|Darbas]]',
  website: 'darbs.lt',
  refName: 'darbs'
});
